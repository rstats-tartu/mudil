...
```{r}
ggplot(ml1, aes(age, length)) + geom_jitter(size = 0.5, alpha = 0.2)+ 
  facet_grid(Location~Sex) + ggthemes::theme_tufte()
```
 logistic fun
a/(1 + b * exp(-k * t))
...
```{r}
ml1$Introduction %>% as.factor() %>% table()
```

Kas introduktsiooniaasta mõjutab kasvu?
...
Introduktsiooniaasta mõju kasvule - paistab puuduvat?
...
```{r}
ES_k_05.16 <- introd_m$`r_Introduction__k[2005,Intercept]` - introd_m$`r_Introduction__k[2012,Intercept]`
rethinking::HPDI(ES_k_05.16, prob = 0.9)
```
...
Nagu näha, lokatsioonide eraldi modelleerimine annab tunduvalt laiema veahinnangu.
...
Kuidas oleks kasvu erinevus 1 aastastel kaladel Kihnus?
Sex + a/(1 + b * exp(-k * age)
```{r}
...
Kolme aastastel ei paista kasv erinevat.
